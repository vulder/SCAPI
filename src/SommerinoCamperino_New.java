...
public class SommerinoCamperino_New extends DefaultBWListener {
    ...
    private void alignUnits(Game state, List<Unit> units, Position pos) {
	if (units.size() < 2)
	    return;
	...
	game.drawTextScreen(400, 10, "State: " + state + " "+ i++ + " " + game.getFrameCount());
	...
	case INIT:
	    alignUnits(game, self.getUnits(),
		    enemy.getStartLocation().toPosition());
