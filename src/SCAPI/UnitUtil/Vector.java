...
public class Vector {
	...
	public Vector normalized() {
		...
		return new Vector(origin,
				new Position(origin.getX() + v.toPosition().getX(), origin.getY() + v.toPosition().getY()));
	}
	...
	public Vector add(Vector rhs) {
		...
		return new Vector(origin, new Position(np.getX() + origin.getX(), np.getY() + origin.getY()));
	}
	...
	public Vector scale(double scale) {
		Vector v = toOrigin();
		Position p = v.toPosition();
		Position np = new Position((int) (p.getX() * scale), (int) (p.getY() * scale));
		return new Vector(origin, new Position(np.getX() + origin.getX(), np.getY() + origin.getY()));
	}

	public Vector toOrigin() {
		Position p = toPosition();
		return new Vector(new Position(0, 0), new Position(p.getX() - origin.getX(), p.getY() - origin.getY()));
	}
	...
	public Vector rotate(double angle) {
		...
		Position rotated = new Position((int) (p.getX() * Math.cos(newAngle) - p.getY() * Math.sin(newAngle)),
				(int) (p.getX() * Math.sin(newAngle) + p.getY() * Math.cos(newAngle)));
		Position shifted = new Position(rotated.getX() + origin.getX(), rotated.getY() + origin.getY());
